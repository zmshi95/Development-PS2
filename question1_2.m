...
table2=importdata('table2.txt');

%draw z for n's households
z=randn(n,1)*sigma_u; % generate lnU
z=exp(z)*exp(-0.5*(sigma_e^2)); % generate z

%draw stochastic component for different months
u0=zeros(size(table2,1),1);
m=[];
for i=1:3
    m(:,i)=mvnrnd(u0,diag(table2(:,i)))';
end

M=exp(m).*exp(-0.5.*m);%stochastic component
...
for i=1:3;
    for j=1:n
    c(i,j,:,:)=z(j)*(e*(M(:,i).*table2(:,i))');%i=1: middle i=2: high; i=3: low
    end
end
...
for i=1:3;
    for j=1:n
    c_nonseason(i,j,:,:)=z(j)*(ones(40,1)*(M(:,i).*table1(:,i))');%i=1: middle i=2: high; i=3: low
    end
end
...
%comments:
%from the results we can see, remove any seasonal or nonseasonal component will increase social welfare
%and the higher eta, the higher welfare coming from of the removal.
% And the magnitute of welfare gain between removal of seasonal and
% nonseasonal is ambiguous.
